...
class ModificarArreglo(Instruccion):
    ...
    def interpretar(self, tree, table,jconsola):
        value = self.valor.interpretar(tree, table,jconsola) # Valor a asignar a la variable
        ...
        value = self.modificarDimensiones(tree, table, copy.copy(self.expresiones), simbolo.getValor(), value,jconsola)     #RETORNA EL VALOR SOLICITADO
        if isinstance(value, Excepcion): return value
    ...
    def modificarDimensiones(self, tree, table, expresiones, arreglo, valor,jconsola):
        ...
        value = self.modificarDimensiones(tree, table, copy.copy(expresiones), arreglo[num], valor,jconsola)
        if isinstance(value, Excepcion): return value
        if value != None:
            arreglo[num] = value
